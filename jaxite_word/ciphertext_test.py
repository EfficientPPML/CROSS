...
class FiniteFieldTest(parameterized.TestCase):
  ...
  def test_montgomery_single_moduli_context(self):
    ...
    single_a_mont = context.to_computation_format(self.single_a.astype(jnp.uint64))
    single_b_mont = context.to_computation_format(self.single_b.astype(jnp.uint64))
    single_ab_mont = single_a_mont.astype(jnp.uint64) * single_b_mont.astype(jnp.uint64)
  ...
  def test_shoup_single_moduli_context(self):
    ...
    single_a_precomputed = context.precompute_constant_operand(self.single_a.astype(jnp.uint64))
    single_ab = self.single_a.astype(jnp.uint64) * self.single_b.astype(jnp.uint64)
  ...
  def test_ntt_conversion(self):
    ...
    parameters = {
      "r": 4,
      "c": 4,
      "finite_field_context": ff_context.BarrettContext, # ff_context.BarrettContext, ff_context.MontgomeryContext, ff_context.ShoupContext
    }
...
class CiphertextArithmeticTest(parameterized.TestCase):
  ...
  def test_mod_reduce(self):
    ...
    expected = self.ct1.ntt_ctx.ff_ctx.modular_reduction(self.ct1.ciphertext).astype(self.ct1.modulus_dtype)

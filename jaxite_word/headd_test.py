"""Private add-kernel tests over Barrett and Montgomery backends."""
...
class HEAddKernelTest(parameterized.TestCase):
  ...
  def _run(self, a, b, ff_ctx_cls):
    op = headd._HEAddKernel(self.q_towers, finite_field_context=ff_ctx_cls)
    out = op.add(self._build_ct(a, ff_ctx_cls), self._build_ct(b, ff_ctx_cls))
    ctx = ff_ctx_cls(self.q_towers)
    return np.asarray(
  ...
  def test_rns_ground_truth(self, ff_ctx_cls):
    ...
    q = np.array(self.q_towers, dtype=np.uint64)
    expected = ((a + b) % q).astype(np.uint64)

"""Performance tests for hesub.py using profiler.py."""
...
def _hesub_kernel(ct_data, parameters):
  """Profile the private subtract kernel under its reduction backend."""
  op = parameters["hesub_op"]
  ct1 = parameters["ct1"]
  ct2 = parameters["ct2"]
  ct1.polynomial = ct_data
  ct2.polynomial = ct_data
  return op.sub(ct1, ct2).polynomial


class HESubKernelPerformanceTest(parameterized.TestCase):
  """Profiles the private subtract kernel."""

  def setUp(self):
    super().setUp()
    require_tpu(self, "hesub")
    self.output_trace_root, self.profiler_config = kernel_perf_setup(__file__)

  @classmethod
  def tearDownClass(cls):
    super().tearDownClass()
    collect_module_logs(__file__, "hesub_profiling", tpu_only=True)

  @parameterized.named_parameters(*TEST_PARAMS)
  def test_hesub(self, degree, r, c, limbs, dnum, moduli, extend_moduli, perf_test):
    num_elements = 2
    degree_layout = (r, c)

    profiler_instance = Profiler(
        output_trace_path=self.output_trace_root,
        profile_naming=f"hesub_N{degree}",
        configuration=self.profiler_config,
    )

    batch_list = [1]
    for red_name, ff_ctx_cls in REDUCTION_CONTEXTS:
      op = hesub._HESubKernel(moduli, finite_field_context=ff_ctx_cls)
      for batch in batch_list:
        ...
        kernel_name = f"hesub_{red_name}_B{batch}"
        kernel_wrapper = KernelWrapper(
            kernel_name=kernel_name,
            function_to_wrap=_hesub_kernel,
            input_structs=[((batch, num_elements, r, c, limbs), jnp.uint32)],
            parameters={"hesub_op": op, "ct1": ct1, "ct2": ct2},
        )

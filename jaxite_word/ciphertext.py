...
def gen_power_of_inv_psi_arr(moduli, ring_dim):
  ...
  power_of_inv_psi_arr = [
      [pow(inv_psi[idx], i, q_list[idx]) for i in range(ring_dim)] for idx in range(len(psi_list))
  ]
  return jnp.array(power_of_inv_psi_arr, jnp.uint64).T.reshape(1, 1, ring_dim, -1)
...
class Ciphertext:
  ...
  #####################
  def modulus_switch_control_gen(self, degree_layout: tuple = None, perf_test: bool = False) -> dict:
    if degree_layout is None:
      degree_layout = (self.degree, )
    if self.num_moduli <= 1:
      raise ValueError("Cannot perform modulus switch with fewer than two moduli.")

    ring_dim = self.degree
    overall_psi = [util.root_of_unity(2 * ring_dim, q) for q in self.moduli]
    overall_power_of_psi = jnp.array(
        [
            [pow(overall_psi[idx], i, self.moduli[idx]) for i in range(ring_dim)]
            for idx in range(len(self.moduli))
        ],
        jnp.uint64,
    )

    gammas, betas = util.gamma_beta_calculation(self.moduli, perf_test=perf_test)
    ...
    ct_last_limb_shapes = {'batch': self.batch, 'num_elements': 4, 'degree': ring_dim, 'precision': 32, 'num_moduli': 1, 'degree_layout': degree_layout}
    self.ct_last_limb = Ciphertext(ct_last_limb_shapes, parameters={'moduli': [self.moduli[-1]], 'finite_field_context': ff_context.BarrettContext, 'r': self.r, 'c': self.c})

    self.gammas_power_of_psi_no_last = jnp.array(gammas_power_of_psi_no_last, jnp.uint64).T.reshape(1, *degree_layout, self.num_moduli - 1)
    ...
    self.power_of_last_tower_inv_psi = jnp.array(power_of_inv_psi_arr_last_tower, jnp.uint64).reshape(1, *degree_layout, 1)

    self.moduli_no_last = jnp.array(self.moduli[:-1], dtype=self.modulus_dtype)
    self.moduli_threshold = jnp.array((self.moduli[-1] + 1) // 2, dtype=jnp.uint32)
  ...
  def rescale(self):
    ...
    ct_last_limb_modred = self.ct_last_limb.get_batch_ciphertext().astype(jnp.uint32)

    condition = ct_last_limb_modred < self.moduli_threshold
    result_if_lt_threshold = ct_last_limb_modred
    result_if_ge_threshold = jnp.array(self.drop_last_moduli_arr, jnp.uint64) - self.last_tower_moduli + ct_last_limb_modred
    ...
    self.mul(self.betas)
    self.set_batch_ciphertext(self.get_batch_ciphertext() + mod_reduce_last_res_unreduced.astype(jnp.uint64))
    self.mod_reduce()
  ...
  #####################
  def ciphertext_mult(self) -> jnp.ndarray:
    ...
    mul0_t = a0 * b0
    mul0 = self.ntt_ctx.ff_ctx.modular_reduction(mul0_t).astype(self.modulus_dtype)

    mul2_t = a1 * b1
    mul2 = self.ntt_ctx.ff_ctx.modular_reduction(mul2_t).astype(self.modulus_dtype)

    t1_t = a0 * b1 + a1 * b0
    mul1 = self.ntt_ctx.ff_ctx.modular_reduction(t1_t)

    return jnp.concatenate([mul0[:, None], mul1[:, None]], axis=1), mul2[:, None]

  #####################
  # Key Switching Functions
  #####################
  def key_switch_control_gen(self, extend_moduli: List[int], dnum: int, evalkey_a, evalkey_b, perf_test: bool = False, selected_moduli: List[int] = None, degree_layout: tuple = None):
    """
    ...
    drop_last_psi = [util.root_of_unity(2*ring_dim, q) for q in drop_last_extend_moduli]
    if perf_test:
      self.ks_params['drop_last_power_of_psi'] = util.random_parameters((*self.degree_layout, len(drop_last_extend_moduli)), drop_last_extend_moduli, dtype=jnp.uint64)
    else:
      self.ks_params['drop_last_power_of_psi'] = jnp.array([
        [pow(drop_last_psi[idx], i, drop_last_extend_moduli[idx]) for i in range(ring_dim)]
        for idx in range(len(drop_last_extend_moduli))
      ], jnp.uint64).T.reshape(*self.degree_layout, len(drop_last_extend_moduli))
    ...
    select_tower_index_overall, non_select_tower_index_overall, restore_indices = [], [], []
    for part in range(self.ks_params['numPartQl']):
      select_tower_overall_index = original_moduli_extract_index[part]
      non_select_tower_overall_index = [i for i in range(len(drop_last_extend_moduli)) if i not in select_tower_overall_index]
      ...
      select_tower_index_overall.append(jnp.array(select_tower_overall_index, jnp.uint16))
      non_select_tower_index_overall.append(jnp.array(non_select_tower_overall_index, jnp.uint16))
    ...
    self.ks_params['non_select_tower_index_overall'] = non_select_tower_index_overall
    ...
    def gen_power_of_inv_psi_arr(moduli):
      ...
      return jnp.array(power_of_inv_psi_arr, jnp.uint64).T.reshape(1, 1, *degree_layout, -1)

    if perf_test:
        self.ks_params['power_of_inv_psi_arr_drop_last'] = util.random_parameters((len(current_moduli), ring_dim), current_moduli, dtype=jnp.uint64).T.reshape(1, 1, *degree_layout, -1)
    else:
        self.ks_params['power_of_inv_psi_arr_drop_last'] = gen_power_of_inv_psi_arr(current_moduli)
    ...
    self.ks_params['evk_a_precomp'] = jnp.concatenate([evalkey_a[..., :idx_cur_last_tower], evalkey_a[..., -overall_sizeP:]], axis=-1).reshape(-1, *self.degree_layout, len(drop_last_extend_moduli))
    self.ks_params['evk_b_precomp'] = jnp.concatenate([evalkey_b[..., :idx_cur_last_tower], evalkey_b[..., -overall_sizeP:]], axis=-1).reshape(-1, *self.degree_layout, len(drop_last_extend_moduli))
  ...
  def key_switch(self):
    ...
    if not hasattr(self, 'ks_params'):
        raise RuntimeError("Key switch parameters not initialized. Call key_switch_control_gen first.")
    ...
    for part in range(numPartQl):
        ...
        _power_of_psi_arr = jnp.take(params['drop_last_power_of_psi'], non_select_idxs, axis=-1)
        ...
        partsCtExt_cur_part = jnp.concatenate(
            [jnp.take(input_ntt, select_idxs, axis=-1), partsCtCompl_multi_moduli],
            axis=-1
        )
        partsCtExt_cur_part = jnp.take(partsCtExt_cur_part, restore_indices[part], axis=-1)

        if res0 is None:
          res0 = partsCtExt_cur_part * evk_b_precomp.astype(jnp.uint64)[part][None, None, :, :]
          res1 = partsCtExt_cur_part * evk_a_precomp.astype(jnp.uint64)[part][None, None, :, :]
        else:
          res0 += partsCtExt_cur_part * evk_b_precomp.astype(jnp.uint64)[part][None, None, :, :]
          res1 += partsCtExt_cur_part * evk_a_precomp.astype(jnp.uint64)[part][None, None, :, :]

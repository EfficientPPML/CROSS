...
def mat_1d_shuffle_to_2d(coefMap, r, c):
    ...
    Perform 1D data shuffing of O(N) in matrix fashion with O(sqrt(N)) memory cost.
    Precomputes the 2D indices. 
    coefMap is the 1D permuted indices. 

    Factor coefMap (length r*c) into row_perm (len r) and col_perm (len c) such that:
    ...
    if coefMap.ndim != 1 or coefMap.shape[0] != r * c:
        raise ValueError(f"coefMap must be 1D of length r*c. Got shape {coefMap.shape}, r*c={r*c}.")
    if r <= 0 or c <= 0:
    ...
    if not np.array_equal(coef2d_h, expected_h):
        raise ValueError(
            "coefMap is NOT decomposable into a single global row permutation + column permutation "
            f"for r={r}, c={c}. (i.e., not P_row ⊗ P_col). "
            "Pick a different (r,c) factorization, or keep using jnp.take(a, coefMap, axis=2)."
        )
...
class HERot:
  def __init__(self, r, c, dnum, rotate_in_ciphertext_moduli, extend_moduli):
    ...
    self.overall_moduli_init = self.rotate_in_ciphertext_moduli + self.extend_moduli
  ...
  def control_gen(self, batch=1, degree_layout=None, perf_test=False):
    ...
    # External Input
    if perf_test:
      power_of_psi = util.random_parameters((*degree_layout, len(overall_moduli)), overall_moduli, dtype=jnp.uint64)
      power_of_inv_psi = util.random_parameters((*degree_layout, len(overall_moduli)), overall_moduli, dtype=jnp.uint64)
    else:
      original_psi = [util.root_of_unity(2 * ring_dim, q) for q in overall_moduli]
      power_of_psi = jnp.array([
          [pow(original_psi[idx], i, overall_moduli[idx]) for i in range(ring_dim)]
          for idx in range(len(overall_moduli))
      ], jnp.uint64).T.reshape(*degree_layout, len(overall_moduli))
      inv_psi = [pow(psi, -1, q) for (q, psi) in zip(overall_moduli, original_psi)]
      power_of_inv_psi = jnp.array([
          [pow(inv_psi[idx], i, overall_moduli[idx]) for i in range(ring_dim)]
          for idx in range(len(overall_moduli))
      ], jnp.uint64).T.reshape(*degree_layout, len(overall_moduli))
    ...
    target_moduli = [item for item in overall_moduli if item not in current_moduli]
    ...
    self.PInvModq = jnp.asarray(PInvModq_approx_down, dtype=jnp.uint32).reshape(sizeQl_in)
    self.power_of_psi = jnp.array(power_of_psi, jnp.uint64)
    self.power_of_inv_psi = power_of_inv_psi[..., :sizeQl_in]
    self.power_of_inv_psi_approx_down = power_of_inv_psi[..., sizeQl_in:sizeQlP_in]
    ...
    for part in range(numPartQl):
      ...
      non_sel_index = [i for i in range(len(overall_moduli)) if i not in sel_index]
    ...
    ct_in_shapes = {'batch': batch, 'num_elements': 1, 'degree': ring_dim, 'precision': 32, 'num_moduli': sizeQl_in, 'degree_layout': degree_layout}
    self.ct_in = Ciphertext(ct_in_shapes, parameters={'moduli': overall_moduli[:sizeQl_in], "BAT_lazy": False})
    ...
    for part in range(numPartQl):
        ...
        ct_part_shapes = {'batch': batch, 'num_elements': 1, 'degree': ring_dim, 'precision': 32, 'num_moduli': _num_moduli_part, 'degree_layout': degree_layout}
        self.ct_parts.append(Ciphertext(ct_part_shapes, parameters={'moduli': _target_moduli_list, "BAT_lazy": False}))

    ct_full_shapes = {'batch': batch, 'num_elements': 1, 'degree': ring_dim, 'precision': 32, 'num_moduli': sizeQlP_in, 'degree_layout': degree_layout}
    self.ct_full = Ciphertext(ct_full_shapes, parameters={'moduli': self.overall_moduli, "BAT_lazy": False})

    ct_approx_shapes = {'batch': batch, 'num_elements': 1, 'degree': ring_dim, 'precision': 32, 'num_moduli': sizeQlP_in - sizeQl_in, 'degree_layout': degree_layout}
    self.ct_approx = Ciphertext(ct_approx_shapes, parameters={'moduli': self.extend_moduli, "BAT_lazy": False})
  ...
  def rotate(
    ...
    select_tower_index, non_select_tower_index = self.select_tower_index, self.non_select_tower_index
    ...
    for part in range(dnum):
      ...
      non_select_tower_index_arr = jnp.array(non_select_tower_index[part], jnp.uint16)
      power_of_psi_arr_part = jnp.take(power_of_psi, non_select_tower_index_arr, axis=-1)

      input_for_bconv = jnp.take(partCtCloneCoef, select_tower_index_arr, axis=-1)
      ...
      partsCtExt_cur_part = jnp.concatenate(
          [jnp.take(in_tower, select_tower_index_arr, axis=-1), partsCtCompl_multi_moduli],
          axis=-1
      )
      partsCtExt_cur_part = jnp.take(partsCtExt_cur_part, self.restore_indices[part], axis=-1)
      if res0 is None:
        res0 = partsCtExt_cur_part * self.evalkey_b_vector[part][None, None, ...]
        res1 = partsCtExt_cur_part * self.evalkey_a_vector[part][None, None, ...]
      else:
        res0 += partsCtExt_cur_part * self.evalkey_b_vector[part][None, None, ...]
        res1 += partsCtExt_cur_part * self.evalkey_a_vector[part][None, None, ...]
    ...
    for element_index in range(keyswitch_core_res.shape[1]):
      _current_slice = approx_down_in_jax[:, element_index:element_index+1, ..., sizeQl:(sizeQl + sizeP)]
      ...
      current_approx_down_in = approx_down_in_jax[:, element_index:element_index+1, ..., :sizeQl]
    ...
    ks_results = jnp.take(ks_results.reshape(-1, 2, ring_dim, sizeQl), self.coefMap, axis=2)

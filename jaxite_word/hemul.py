...
import util
import numpy as np
from finite_field import BarrettContext
jax.config.update("jax_enable_x64", True)


class HEMul:
  def __init__(self, batch, r, c, dnum, num_eval_mult, original_moduli, extend_moduli):
    self.batch = batch
    self.r = r
    self.c = c
    self.dnum = dnum
  ...
  def control_gen(self, degree_layout=None, perf_test=False):
    ...
    self.overall_sizeQ_in, self.overall_sizeP_in = overall_sizeQ_in, overall_sizeP_in
    ...
    power_of_last_tower_psi = jnp.array([pow(last_tower_psi, i, self.last_tower_moduli) for i in range(ring_dim)], jnp.uint64)
    last_tower_inv_psi = pow(last_tower_psi, -1, self.last_tower_moduli)
    self.power_of_last_tower_inv_psi = jnp.array([pow(last_tower_inv_psi, i, self.last_tower_moduli) for i in range(ring_dim)], jnp.uint64).reshape(1,1,-1)
    self.moduli_threshold = jnp.array((self.last_tower_moduli + 1) // 2, dtype=jnp.uint32)
    ...
    # ==========================================================================
    ct_refer_shapes = {'batch': self.batch, 'num_elements': 4, 'degree': ring_dim, 'precision': 32, 'num_moduli': overall_sizeQ_in, 'degree_layout': self.degree_layout }
    self.ct_refer = Ciphertext(ct_refer_shapes, parameters={'moduli': self.original_moduli, 'finite_field_context': BarrettContext, 'r': self.r, 'c': self.c})
    ...
    self.ct_obj = Ciphertext(ct_shapes, parameters={'moduli': self.drop_last_moduli, 'finite_field_context': BarrettContext, 'r': self.r, 'c': self.c})

    ct_last_limb_shapes = {'batch': self.batch, 'num_elements': 4, 'degree': ring_dim, 'precision': 32, 'num_moduli': 1, 'degree_layout': self.degree_layout}
    self.ct_last_limb = Ciphertext(ct_last_limb_shapes, parameters={'moduli': [self.last_tower_moduli], 'finite_field_context': BarrettContext, 'r': self.r, 'c': self.c})

    idx_cur_last_tower = overall_sizeQ_in - self.num_eval_mult # Calculate here for control_gen

    ct_extend_shapes = {'batch': self.batch, 'num_elements': 2, 'degree': ring_dim, 'precision': 32, 'num_moduli': overall_sizeP_in, 'degree_layout': self.degree_layout}
    self.ct_extend = Ciphertext(ct_extend_shapes, parameters={'moduli': self.extend_moduli})

    # ==========================================================================
    # 3. Parameter Generation
    # ==========================================================================
    if perf_test:
      power_of_psi = util.random_parameters((len(self.drop_last_extend_moduli), ring_dim), self.drop_last_extend_moduli, dtype=jnp.uint64).T
      power_of_inv_psi_approx_down = util.random_parameters((len(self.drop_last_extend_moduli), ring_dim), self.drop_last_extend_moduli, dtype=jnp.uint64).T
    else:
      extend_psi = [util.root_of_unity(2 * ring_dim, q) for q in self.drop_last_extend_moduli]
      power_of_psi = jnp.array([
          [pow(extend_psi[idx], i, self.drop_last_extend_moduli[idx]) for i in range(ring_dim)]
          for idx in range(len(self.drop_last_extend_moduli))
      ], jnp.uint64).T
      extend_inv_psi = [pow(psi, -1, q) for (q, psi) in zip(self.drop_last_extend_moduli, extend_psi)]
      power_of_inv_psi_approx_down = jnp.array([
          [pow(extend_inv_psi[idx], i, self.drop_last_extend_moduli[idx]) for i in range(ring_dim)]
          for idx in range(len(self.drop_last_extend_moduli))
      ], jnp.uint64).T
    self.power_of_psi = power_of_psi[:, :idx_cur_last_tower]
    self.power_of_inv_psi_approx_down = power_of_inv_psi_approx_down[:, -overall_sizeP_in:]
    self.bconv = BConvBarrett(self.drop_last_extend_moduli)
    control_indices_list = []
    rotate_indices = list(range(idx_cur_last_tower))
    extend_indices = list(range(idx_cur_last_tower, idx_cur_last_tower + overall_sizeP_in))
    ...
    target_moduli = [item for item in self.drop_last_moduli if item not in current_moduli]
    P = 1
    for moduli in current_moduli:
      P *= moduli
    PInvModq_approx_down = [util.modinv(P, q) for q in target_moduli]
    self.PInvModq = jnp.asarray(PInvModq_approx_down, dtype=jnp.uint32).reshape(idx_cur_last_tower)

    gammas, betas = util.gamma_beta_calculation(self.original_moduli, perf_test=perf_test)
    ...
    # 4. Parameter Reshape
    # ==========================================================================
    self.power_of_psi = self.power_of_psi.reshape(*self.degree_layout, idx_cur_last_tower)
    self.power_of_inv_psi_approx_down = self.power_of_inv_psi_approx_down.reshape(*self.degree_layout, overall_sizeP_in)
    self.gammas_power_of_psi_no_last = self.gammas_power_of_psi_no_last.T
    self.betas = jnp.array(betas, jnp.uint64).reshape(1, 1, 1, -1)
    self.drop_last_moduli_arr = jnp.array(self.drop_last_moduli, jnp.uint32).reshape(1,1,1,-1)

    self.drop_last_extend_moduli_arr = jnp.array(self.drop_last_extend_moduli, jnp.uint32)
    self.q_correction = self.drop_last_extend_moduli_arr[:idx_cur_last_tower].reshape(1, 1, 1, -1)
    self.post_rescale_shape = (self.batch, 4, *self.degree_layout, len(self.drop_last_moduli))
  ...
  def setup_relinearization(self, evalkey_a_vector, evalkey_b_vector):
    ...
    self.evalkey_a_vector = evalkey_a_vector.astype(jnp.uint64)[:, None, *self.degree_layout]
    self.evalkey_b_vector = evalkey_b_vector.astype(jnp.uint64)[:, None, *self.degree_layout]
  ...
  def mul(self, in_ciphertexts):
    ...
    overall_sizeQ_in, overall_sizeP_in = self.overall_sizeQ_in, self.overall_sizeP_in
    ...
    self.ct_extend.set_batch_ciphertext(approx_down_in_jax[..., idx_cur_last_tower:(idx_cur_last_tower + overall_sizeP_in)])
    self.ct_extend.to_coeffs_form()
    self.ct_extend.modmul(jnp.array(self.power_of_inv_psi_approx_down, jnp.uint64))
    ...
    for element_index in range(ct_new_basis_coef.shape[1]):
      tower_new_basis_coef = ct_new_basis_coef[:, element_index:element_index+1, ...]
      self.ct_obj.set_batch_ciphertext(tower_new_basis_coef)
      self.ct_obj.modmul(self.power_of_psi)
      tower_new_basis_coef_scaled_muli_moduli_modq = self.ct_obj.get_batch_ciphertext()

      self.ct_obj.set_batch_ciphertext(tower_new_basis_coef_scaled_muli_moduli_modq)
      self.ct_obj.to_ntt_form()
      tower_new_basis_jax = self.ct_obj.get_batch_ciphertext()

      current_approx_down_in = approx_down_in_jax[:, element_index:element_index+1, ..., :idx_cur_last_tower]
    ...
    val = jnp.where(result >= self.q_correction, result - self.q_correction, result)

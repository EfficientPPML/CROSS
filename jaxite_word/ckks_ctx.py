...
def slot_to_coeffs(y: List[complex]) -> List[float]:
    ...
    Adjust here if your build packs coefficients differently.
    Current: [Re(y_0..y_{Nh-1}), Im(y_0..y_{Nh-1})]
    """
    nh = len(y)
...
def fit_to_native_vector(vec: List[int], big_bound: int, modulus: int, ring_dim: int) -> List[int]:
    """
...
def ckks_encrypt(plaintext: List[List[int]],
                public_key: List[int],
                q_towers: List[int],
    ...
    psi =  None
    if v is None:
      psi = [util.root_of_unity(int(2*degree), q_towers[t_id]) for t_id in range(num_towers)]
      v = kg.gen_ternary_uniform_polynomial(degree, q_towers).coeffs
      v = [util.bit_reverse_array(util.ntt_negacyclic_bit_reverse(v[t_id] , q_towers[t_id], psi[t_id])) for t_id in range(num_towers)]
    ...
    if e is None:
      ...
      e0 = [util.bit_reverse_array(util.ntt_negacyclic_bit_reverse((e0[t_id]), q_towers[t_id], psi[t_id])) for t_id in range(num_towers)]
      e1 = kg.gen_gaussian_polynomial(degree, q_towers, sigma=sigma).coeffs
      e1 = [util.bit_reverse_array(util.ntt_negacyclic_bit_reverse((e1[t_id]), q_towers[t_id], psi[t_id])) for t_id in range(num_towers)]
    ...
    if ba is not None:
        ...
        c0_T = [[(c0[d][m] - plaintext[d][m]) % q_towers[m] for d in range(degree)] for m in range(num_towers)]
...
def ckks_decrypt(ciphertext: List[List[List[int]]],
    ...
    for m in range(num_towers):
        ...
        # intt
        coef = util.intt_negacyclic_bit_reverse(col_rev, q_towers[m], util.root_of_unity(2*degree, q_towers[m]))
...
def ckks_encode(slots: List[complex],
                cycl_order: int,
                q_towers: List[int],
                p_towers: List[int],
                scale: float,
    ...
    elements = [fit_to_native_vector(ints_base, MAX_INT_64, q_tower, N) for q_tower in q_towers]

    # 5) Scale back up by approx_factor (power of two) in the ring
    if log_approx > 0:
        step = 1 << log_approx
        ints = [ [x * step % q_towers[mod_id] for x in elements[mod_id]] for mod_id in range(len(q_towers))]
    else:
        ints = elements
    ...
    Q_res = [util.ntt_negacyclic_bit_reverse(ints[mod_id], q_towers[mod_id], util.root_of_unity(m, q_towers[mod_id])) for mod_id in range(len(q_towers))]
...
def ckks_decode(
  ...
  curValues = [complex(real_part_list[i], imag_part_list[i]) for i in range(slots)]
  ...
  def _stddev(vec: List[complex], conjugate: List[complex]) -> float:
    ...
    mean /= (dslots - 1.0)
    variance = 2 * sum(((cv.real - mean) ** 2 + (cv.imag - mean) ** 2)
  ...
  if (logstd > p - 5.0):
    raise Exception("The decryption failed because the approximation error is too high. Check the parameters. ")
  ...
  if ADD_NOISE:
    curValues = [
      complex(
        real_part_list[i] * scale + conjugate[i].real * scale + powP * _gauss(),
        imag_part_list[i] * scale + conjugate[i].imag * scale + powP * _gauss()
      )
      for i in range(slots)
    ]
  else:
...
def _crt_combine_rns_plaintext(rns_plaintext: List[List[int]], moduli: List[int]) -> List[int]:
    """
...
class CKKSContext:
    ...
    def encrypt(self, plaintext: ct.Ciphertext, v=None, e=None, ba=None, e_ref=None, v_ref=None) -> ct.Ciphertext:
        ...
        c0 = jnp.expand_dims(jnp.array(c_poly[0], dtype=jnp.uint64), axis=0) # (1, degree, moduli)
    ...
    def decrypt(self, ciphertext: ct.Ciphertext) -> ct.Ciphertext:
        ...
        elem = jnp.expand_dims(jnp.array(decrypted_poly_rns, dtype=jnp.uint32), axis=0)
    ...
    def decode(self, encoded_plaintext: ct.Ciphertext, is_ntt: bool = False) -> jnp.ndarray:
        ...
        if is_ntt:
             ...
             for t_id in range(num_towers):
                 ...
                 intt_vals = util.intt_negacyclic_bit_reverse(rev, self.q_towers[t_id], util.root_of_unity(2*self.degree, self.q_towers[t_id]))

                 for d in range(self.degree):
                     new_poly[d][t_id] = intt_vals[d]
             rns_poly = new_poly

        plain_combined = _crt_combine_rns_plaintext(rns_poly, self.q_towers[:num_towers])
    ...
    def multiply(self, ciphertext1: ct.Ciphertext, ciphertext2: ct.Ciphertext) -> ct.Ciphertext:
    ...
    def add(self, ciphertext1: ct.Ciphertext, ciphertext2: ct.Ciphertext) -> ct.Ciphertext:
        raise NotImplementedError("Add function not implemented")

    def sub(self, ciphertext1: ct.Ciphertext, ciphertext2: ct.Ciphertext) -> ct.Ciphertext:

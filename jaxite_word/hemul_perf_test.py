...
def _hemul_mul_kernel(input_array, parameters):
  ...
  return parameters["he_mul"].mul(input_array)
...
class PerformanceTest(parameterized.TestCase):
  ...
  def _create_kernel_wrapper(self, kernel_name, he_mul, batch, elements, degree_layout, limbs):
...
class BatchDimensionShardingTest(parameterized.TestCase):
  ...
  def _create_sharded_kernel_wrapper(self, kernel_name, he_mul, batch, elements, degree_layout, limbs, mesh, batch_sharding):
    input_shape = (batch, elements, *degree_layout, limbs)
    return KernelWrapper(
        kernel_name=kernel_name,
        function_to_wrap=_hemul_mul_kernel,

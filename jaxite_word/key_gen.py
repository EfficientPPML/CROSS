...
# Key Generation
########################
def gen_ternary_uniform_polynomial(degree: int, moduli: list[int]) -> RnsPolynomial:
...
def _validate_private_key(private_key: List[List[int]]) -> Tuple[int, int]:
    ...
    for row in private_key:
        if not isinstance(row, list) or not row:
            raise ValueError("private_key must be a non-empty 2D list with non-empty rows")
        if degree is None:
            degree = len(row)
        elif len(row) != degree:
            raise ValueError("All rows in private_key must have the same length (degree)")
        for coeff in row:
...
def modulus_switch(coefficient: int | List[int],
                  cur_moduli: int = 524353,
                  target_moduli: int = 1152921504606845473) -> int | List[int]:
    """
...
def gen_evaluation_key(
    ...
    noise_scale: int = 1,
    a: List[List[List[int]]] = None,
    e: List[List[List[int]]] = None,
    dnum: int = 3,
) -> Dict[str, Any]:
    ...
    sOld = [[_mod_q(sk_q[i][j] * sk_q[i][j], q_list[i]) for j in range(degree)] for i in range(size_q)]
...
def key_switch_gen(
    ...
    degree = len(sOut[0])
    s_qp: List[List[int]] = [list(row)[:] for row in sOut]
    for q_p in p_list:
    ...
    s_qp_eva = []
    for (limb, q) in zip(s_qp, q_list + p_list):
        psi = util.root_of_unity(2 * degree, q)
        temp = util.ntt_negacyclic_bit_reverse(limb, q, psi)
        s_qp_eva.append(util.bit_reverse_array(temp))
    P_prod = 1
    for p in p_list:
        P_prod *= p
    ...
    for part in range(num_part_q):
        ...
        a_rows = [util.bit_reverse_array(util.ntt_negacyclic_bit_reverse(a_sample.coeffs[i], modulus_i, util.root_of_unity(int(degree<<1), modulus_i))) for i, modulus_i in enumerate(moduli_list[:size_qp])]
        e_sample = gen_gaussian_polynomial(degree, moduli_list[:size_qp], sigma=noise_std)
        e_rows = [util.bit_reverse_array(util.ntt_negacyclic_bit_reverse(e_sample.coeffs[i], modulus_i, util.root_of_unity(int(degree<<1), modulus_i))) for i, modulus_i in enumerate(moduli_list[:size_qp])]
...
def gen_rotation_key(sk, original_moduli, extend_moduli, rot_index, dnum=3, noise_std=3.190000057220458984375, noise_scale=1, a=None, e=None):
  n = len(sk[0])
  ...
  ek = key_switch_gen(sk, sNew=sk_rot, q_list=original_moduli, p_list=extend_moduli, noise_std=noise_std, noise_scale=noise_scale, a=a, e=e, dnum=dnum)
  return ek
...
def gen_pke_pair(
    ...
    s = [util.bit_reverse_array(util.ntt_negacyclic_bit_reverse(s.coeffs[i], modulus_i, util.root_of_unity(int(degree<<1), modulus_i))) for i, modulus_i in enumerate(moduli_list)]
    s = s_ref if s_ref is not None else s
    a = gen_uniform_polynomial(degree, moduli_list)
    a = [util.bit_reverse_array(util.ntt_negacyclic_bit_reverse(a.coeffs[i], modulus_i, util.root_of_unity(int(degree<<1), modulus_i))) for i, modulus_i in enumerate(moduli_list)]
    a = a_ref if a_ref is not None else a
    e = gen_gaussian_polynomial(degree, moduli_list, sigma=noise_std)
    e = [util.bit_reverse_array(util.ntt_negacyclic_bit_reverse(e.coeffs[i], modulus_i, util.root_of_unity(int(degree<<1), modulus_i))) for i, modulus_i in enumerate(moduli_list)]
